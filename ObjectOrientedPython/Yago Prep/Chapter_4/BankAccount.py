        ...
        if password != self.password: 
            ...
            if (externalAccNumber > 0 & externalAccNumber < 1000000): 
                if amountToTransfer < self.balance: 
                    ...
                    print(f'Transaction to Acc. Number {externalAccNumber} of ${amountToTransfer} has been successfully done.')
        ...
        if amountToWithdraw > 0: 
            if password != self.password: 
                ...
                if amountToWithdraw < self.balance: 
                    ...
                    print(f'${amountToWithdraw} successfully withdrawn. Please take your money\nRemaining balance: {self.balance}')
                else: 
                    print(f'ERROR: Unfortunately you are exceeding your withdrawal capabilities\nAvailable balance: {self.balance}\nFeel fry to try again')
        else: 
    ...
    def depositMoney(self, amountToDeposit, password):
        if amountToDeposit > 0:
            if password != self.password: 
                ...
                print(f'${amountToDeposit} correctly deposited\nNew Balance: ${self.balance}')
        else: 
